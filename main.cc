...
vector<string> key_word_yes {"YES","SURE","OKAY","DEFINITELY","ACCEPT","FINE","YEAH","YA"};
vector<string> key_word_no {"NO","DONT","DON'T","NOPE","NAH"};
vector<string> key_word_launch {"LAUNCH","TAKE OFF","FLY","GO","NAVIGATE","NAVIGATION","WARP","TAKE US OUT","HEAD OUT","HEAD","DRIVE","TRAVEL","LEAVE"};
vector<string> key_word_advice {"HELP","ADVICE","IDEA","ADVISE"};
vector<string> key_word_battle {"ATTACK","BATTLE","SHOOT","FIRE","ENGAGE","FIGHT"};
...
vector<string> hull{"HULL","BODY","STRUCTURAL","STRUCTURE","INTEGRITY"};
...
void ADVICE() {
        ...
        TYPER("The enemy shields protect the hull quite effectively");
        TYPER("so you might want to prioritize them.");
        TYPER("If you take evasive maneuvers, your engines will be reduced ");
        TYPER("by 10% and your shields will be recharged by a small margin."); NarrP();
        cout << endl;
}
...
"                                              \\/	             ",
	...
	//Puzzle-1
        ...
        vector<string> correct; correct.push_back("LITTLE");
        string test = "";
        bool loop = false;
        string prompt = "";
        while (!loop){
                ...
                if (WRDSRCH(test, correct)){
                        ...
                        else if (WRDSRCH(action, key_word_no)){
                                continue;
                        }
                        else {TYPER("That did not compute."); continue;}
                }
        }
	...
	elipse(); cout << endl;
	TYPER("You decide to help get the engines started"); elipse();
	...
	//Puzzle-2
        ...
        string ans2;
        string ans3;
        TYPER("For each passcode you will be asked a simple math riddle to make sure that you"); TYPER("are competent enough to operate the fuel mixology.\n"); NarrP();
        TYPER("The first riddle is:");
        while (!loop1){
                TYPER("What number multiplied by itself and divided by one hundred is equal to four?");
                getline(cin, ans1);
                for (int i = 0; i < ans1.size(); i++) {
                        if (!isdigit(ans1.at(i))) {
                                TYPER("Invalid Input"); break;
                        }
                }
                if (ans1 == "20"){
	...
	TYPER("if we are going to make it to Andromeda!"); NarrP(); cout << endl;
	TYPER("The commander seems available to offer advice.");
	elipse(); cout << endl;
	while (true) {
	TYPER("What would you like to do?");
	getline(cin, action);
	...
	//Puzzle-3
        bool loopP3 = false;
        string ansT;
        vector<string> correctT {"COLD"};
        TYPER("To enter Titan's orbit you must answer a riddle to show that you are an "); TYPER("intelligent member of society and someone who it is worth doing business with.\n");NarrP();
        TYPER("The riddle is as follows:");NarrP();
        while (!loopP3){
                TYPER("What can you catch but never throw?");
                getline(cin, ansT);
                TOUPPER(ansT);
                if(WRDSRCH(ansT, correctT)){
                        TYPER("Thats it!\n");NarrP();
                        loopP3 = true;
                }else { TYPER("Thats not quite it please try again.\n");}
        }
	...
	TYPER("You explain that you are carrying out an important mission for Star Command, but your ");
	TYPER("warp-drive is offline due to a collision with an asteroid."); NarrP();
	TYPER("After the head presses further questions on you, you are forced to explain that you are ");
	TYPER("carrying valuable cargo to andromeda."); NarrP();
	TYPER("Bezos gives a look and nod to one of his guards who proceeds to leave the room."); NarrP();	
	...
	//Puzzle-4
        ...
        bool loopP4 = false;
	TYPR("BEZOS: "); NarrP();
        TYPR("If you want my goods you must prove your worth"); elipse(); cout << endl; 
	TYPER("I know! "); NarrP(); TYPER("how about you answer a simple riddle.\n"); NarrP();
        while (!loopP4){
                TYPER("What is Black and White and Red all over?\n1) A Newspaper?\n2) A sunburnt cow?\n3) A sunburnt Zebra");NarrP();
                getline(cin, ansBEZ);
                for (int i = 0; i < ansBEZ.size(); i++) {
                        if (!isdigit(ansBEZ.at(i))) {
                                TYPER("Invalid Input"); break;
                        }
                }
                if(ansBEZ == "3"){
	...
	//Puzzle-5
        bool loopP5 = false;
        string ansWARP;
        TYPR("CHIEF OF ENGINEERING: "); NarrP(); TYPER("There's a lot to to get this thing back in top shape."); cout << endl;
	TYPER("The chief of engineering hands you an instruction manual and assigns you to aid in repairs."); NarrP();
	TYPR("Decifering the manual feels a lot like a number game to you"); elipse(); cout << endl;
	while (!loopP5){
                TYPER("What number divided by five, multiplied by seven, and divided by six is equal to seven?");
                getline(cin, ansWARP);
                for (int i = 0; i < ansWARP.size(); i++) {
                        if (!isdigit(ansWARP.at(i))) {
                                TYPER("Invalid Input"); break;
                        }
                }
                if (ansWARP == "30"){
	...
	while (quit == 0) {
			...
			TYPR("I'm suspicious about that Bezos guy"); elipse(); cout << endl;
			TYPR("We may have just opened a can of worms"); elipse(); cout << endl;
                }
